...
class Alfabet(Frame):
    ...
    def canAlfabet(self):
        ...
        imageobj = Image.open(os.path.join(imgfolder, 'alf.jpg'))
        imageobj = imageobj.resize((900, 300), Image.ANTIALIAS )
        self.photoimg = ImageTk.PhotoImage(imageobj)
        can = Canvas()
        can.pack(fill=BOTH)
        can.config(width=self.photoimg.width(), height= self.photoimg.height(), bg='blue')
        can.create_image(2, 2, image= self.photoimg, anchor=NW)
...
# entry widget, where user gives his/her answer
    ...
    def onfetch(self):
        ...
        if self.numtry > 0:
            ...
            if self.numtry > 1:
                ...
                can.create_image(2, 2, image= endimg, anchor=NW)
    ...
    def onRestart(self):
        ...
        self.score, self.numtry, self.allnumtry = 0, self.num.get(), self.num.get()
        self.win.destroy()
